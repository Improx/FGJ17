...
public class WaveVisualizer : MonoBehaviour {
    ...
    private void OnDrawGizmos() {
        if (targetC != null) {
            ...
            Gizmos.DrawLine(transform.position, targetC.position);
        }
        if (targetL != null) {
            Gizmos.color = Color.green;
            Gizmos.DrawLine(transform.position, targetL.position);
        }
        if (targetR != null) {
            Gizmos.color = Color.green;
            Gizmos.DrawLine(transform.position, targetR.position);
        }
    }
}
